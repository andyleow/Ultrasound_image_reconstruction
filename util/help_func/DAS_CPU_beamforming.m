...
%  Eg : reconImg = DAS_CPU_Func(rf_data, deg_tx, actAperture,filterCoef, pixelMapX, 
%       pixelMapZ,delay,fs,ftx,c,focus,ReconMode)

% Input: 
%   1)rf_data: Input rf, multiPW(sample, channel,angle,frame)/singlePW(sample, channel, frame)
%   2) deg_tx : A vector containing the transmit steering angle (in radian)
%   3) actAperture:A vector containing the aperture position
...
%   7) delay: delay that account for transmit delay, receive delay and lens correction [s]
...
for i = 1:totalFrame
    ...
    image1=DAS_CPU_Func(rf_data,ImagParam.deg_tx, actAperture,FiltParam.coef, pixelMapX,...
