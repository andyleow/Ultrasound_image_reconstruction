...
%             pixelMap.bottomRight = [Trans.position(Trans.startAperture+127,1)*1+pixelMap.dx/2 UserSet.sampleDepthMM/1000];
        ...
        case 'P'
            ...
            aperture = Trans.Num_Element*(Trans.Ele_pitch+Trans.Ele_width); % aperture based on X elements
            pixelMap.focus = -(aperture/2)/tan(UserSet.PAngle);
            pixelMap.dz=1540/(Trans.frequency*4*2);
            pixelMap.dx=Trans.Ele_pitch;
            pixelMap.UpperLeft = [-UserSet.sampleDepthMM 0*1e3]/1000;
            pixelMap.BottomRight = [UserSet.sampleDepthMM UserSet.sampleDepthMM]/1000;
            pixelMap.resolutionH = ceil((pixelMap.BottomRight(2)-pixelMap.UpperLeft(2))/pixelMap.dz);			% 5mm = 33pixel pixel.z
            pixelMap.resolutionW = ceil((pixelMap.BottomRight(1)-pixelMap.UpperLeft(1))/pixelMap.dx);	         %pixel.x
    end
    %% GPU Beamforming
    tic;
    data = Fourier_CPU_beamforming(location, Trans,ImagParam, FiltParam,pixelMap,'on');
    toc
%     return
