...
%Load data for reconstruction sim 2
% global RcvData;
% load('E:\Chee Hau\150723_183038PW_bMODE');
...
UserSet.sampleDepthMM=80;   %[15,20,25,30,35,40,45,50] in mm
...
UserSet.TXFreq=2.72;       %Transmit Frequency(MHz)
UserSet.TXFocusMM= 0;
UserSet.numCycle=1;     %NumHalfCycle
UserSet.na=11;           %Num Compounded PW
UserSet.AngleRange=60;  %Max Range 
UserSet.scanRangle = pi/4;
...
Intermediate.maxAcqLength = sqrt(sampleDepthWL^2 + (Trans.numelements*Trans.spacing)^2);
...
wavelength = Resource.Parameters.speedOfSound/Trans.frequency*1e-6;
P.theta = UserSet.scanRangle;
...
UserSet.TXFocusMM = -P.radius*wavelength*1e3;

PData(1).PDelta = [0.875, 0, 0.25];
PData.Size(1) = ceil(sampleDepthWL)/PData.PDelta(3); % startDepth, endDepth and pdelta set PData.Size.
PData(1).Size(2) = ceil(2*(ceil(sampleDepthWL) + P.radius)*sin(P.theta)/PData(1).PDelta(1));
PData(1).Size(3) = 1;
PData(1).Origin = [-(PData(1).Size(2)/2)*PData(1).PDelta(1),0,0];
PData(1).Region = struct(...
            'Shape',struct('Name','SectorFT', ...
            'Position',[0,0,-P.radius], ...
            'z',0, ...
            'r',ceil(sampleDepthWL), ...
            'angle',P.rayDelta, ...
            'steer',0));
...
% % - Sensitivity Cutoff
UI(1).Control =  {'UserA2','Style','VsSlider','Label','Sens. Cutoff',...
                  'SliderMinMaxVal',[0,1.0,Preset.SenseCut],...
                  'SliderStep',[0.025,0.1],'ValueFormat','%1.3f'};
UI(1).Callback = text2cell('%SensCutoffCallback');
% 
% - Range Change
UIParam.Range=sampleDepthWL;
RangeMinMaxVal = [64,sampleDepthWL,UIParam.Range]; % default unit is wavelength
if isfield(Resource.DisplayWindow(1),'AxesUnits')&&~isempty(Resource.DisplayWindow(1).AxesUnits)
    if strcmp(Resource.DisplayWindow(1).AxesUnits,'mm')
        RangeMinMaxVal = RangeMinMaxVal * wavelength*1e3;
    end
end

UI(2).Control = {'UserA1','Style','VsSlider','Label','Range',...
                 'SliderMinMaxVal',RangeMinMaxVal,'SliderStep',[0.1,0.2],'ValueFormat','%3.0f'};
UI(2).Callback = text2cell('%RangeChangeCallback');
 
% Preset Transmit Voltage
UI(3).Statement ='[result,hv] = setTpcProfileHighVoltage(evalin(''base'',''Preset.Voltage'')'',1);';
UI(4).Statement ='hv1Sldr = findobj(''Tag'',''hv1Sldr'');';
UI(5).Statement ='set(hv1Sldr,''Value'',hv);';
UI(6).Statement ='hv1Value = findobj(''Tag'',''hv1Value'');';
UI(7).Statement ='set(hv1Value,''String'',num2str(hv,''%.1f''));';

UI(8).Control = {'UserB2','Style','VsPushButton','Label','ReconAll'};
UI(8).Callback = text2cell('%ReconRFCallBack');

UI(9).Control = {'UserB1','Style','VsPushButton','Label','saveRF'};
UI(9).Callback = text2cell('%saveRFCallback');
...
save('MatFiles/P4-2vFlashAngles_GPU');
filename='P4-2vFlashAngles_GPU'; VSX;
return
...
if simMode == 2
    set(hObject,'Value',evalin('base','sampleDepthWL'));
    return
end
...
UIParam=evalin('base','UIParam');
UIParam.Range = UIValue;
if isfield(Resource.DisplayWindow(1),'AxesUnits')&&~isempty(Resource.DisplayWindow(1).AxesUnits)
    if strcmp(Resource.DisplayWindow(1).AxesUnits,'mm')
        UIParam.Range = UIValue*scaleToWvl;    
    end
end

assignin('base','UIParam',UIParam);

PData=evalin('base','PData');
P=evalin('base','P');
PData.Size(1) = ceil(UIParam.Range/PData.PDelta(3));
PData.Size(2) = ceil(2*(ceil(UIParam.Range) + P.radius)*sin(P.theta)/PData(1).PDelta(1));
PData.Origin = [-(PData(1).Size(2)/2)*PData(1).PDelta(1),0,0];
PData(1).Region = struct(...
            'Shape',struct('Name','SectorFT', ...
            'Position',[0,0,-P.radius], ...
            'z',0, ...
            'r',ceil(UIParam.Range), ...
            'angle',P.rayDelta, ...
            'steer',0));
...
%Change TGC base on depth
evalin('base','TGC.rangeMax = UIParam.Range;');
evalin('base','TGC.Waveform = computeTGCWaveform(TGC);');
evalin('base','if VDAS==1, Result = loadTgcWaveform(1); end');

Control = evalin('base','Control');
Control.Command = 'update&Run';
Control.Parameters = {'PData','ImageBuffer','DisplayWindow','TGC'};
...
cuDAS_int(1,GPUParams.rf_data(:,:,:,1),GPUParams.deg_tx, GPUParams.actAperture,GPUParams.filterCoef, GPUParams.pixelMapX,...
            GPUParams.pixelMapZ,GPUParams.delay,GPUParams.fs, GPUParams.ftx,GPUParams.c, GPUParams.txFocus,...
            GPUParams.SenseCutoff,GPUParams.reconMode,GPUParams.gpuID);
...
for i=1:GPUParams.numFrame
    ...
    GPUParams.IQData(:,:,i)=cuDAS_int(0,GPUParams.rf_data(:,:,:,i));
end
close (h);
...
if Trans.numelements<Resource.Parameters.numRcvChannels
    ...
    GPUParams.channelInd = Trans.ConnectorES;
else
...
tic;
img = cuDAS_int(0,GPUParams.rf_data);
GPUParams.IQData=img;
% for i=1:sizeRF(4)
%     img = cuDAS_int(0,GPUParams.rf_data(:,:,:,i));
%     GPUParams.IQData(:,:,i)=img;
% end
toc;
